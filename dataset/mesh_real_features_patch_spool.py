...
class FaceGraphMeshDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        semantic_maps = [x.long() for x in pt_arxiv['semantic_maps']]
        ...
        return {
            ...
            "graph_data": self.get_item_as_graphdata(edge_index, sub_edges, pad_sizes, num_sub_vertices, pool_maps, semantic_maps, lateral_maps, is_pad, level_masks)
        }

    @staticmethod
    def get_item_as_graphdata(edge_index, sub_edges, pad_sizes, num_sub_vertices, pool_maps, semantic_maps, lateral_maps, is_pad, level_masks):
        return EasyDict({
    ...
    def process_real_image(self, path):
        ...
        padded = pad(read_image(str(path)).float() / 127.5 - 1)
