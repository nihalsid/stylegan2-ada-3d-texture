...
from model.graph import create_faceconv_input, SmoothUpsample, modulated_face_conv, modulated_texture_conv
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, graph_data, ws, shape, noise_mode='random'):
        ...
        _, face_colors = self.last_block(neighborhoods[0], graph_data['is_pad'][0], graph_data['pads'][0], appended_pool_maps[0], x, face_colors, split_ws[-1], shape[0], noise_mode)
        return face_colors
...
class SynthesisLayer(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, w_dim, num_face, resampler=None, activation='lrelu'):
        super().__init__()
        self.aggregation_function = torch.nn.AvgPool2d(kernel_size=(1, 9))
        self.num_face = num_face
