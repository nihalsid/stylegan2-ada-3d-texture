...
from model.graph import create_faceconv_input, SmoothUpsample, modulated_face_conv, FaceConvDemodulated


class Generator(torch.nn.Module):

    def __init__(self, z_dim, w_dim, w_num_layers, num_faces, color_channels, semantic_channels, c_dim=0, channel_base=16384, channel_max=512):
        super().__init__()
        self.z_dim = z_dim
        self.w_dim = w_dim
        self.c_dim = c_dim
        ...
        self.synthesis = SynthesisNetwork(w_dim=w_dim, num_faces=num_faces, color_channels=color_channels, semantic_channels=semantic_channels, channel_base=channel_base, channel_max=channel_max)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def __init__(self, w_dim, num_faces, color_channels, semantic_channels, channel_base=16384, channel_max=512):
        super().__init__()
        self.w_dim = w_dim
        self.num_faces = num_faces
        self.color_channels = color_channels
        ...
        use_semantic_pooling = {4: False, 8: False, 16: True, 32: True, 64: True, 128: True}
        ...
        for blk_idx, cdict_key in enumerate(self.block_pow_2[1:]):
            ...
            geo_channels = channels_dict_geo[cdict_key // 2]
            next_geo_channels = channels_dict_geo[cdict_key]
            out_channels = channels_dict[cdict_key]
            use_semantics = use_semantic_pooling[cdict_key // 2]
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, current_geo_channels=geo_channels,
                                   next_geo_channels=next_geo_channels, num_face=num_faces[block_level],
                                   color_channels=color_channels, semantic_channels=semantic_channels if use_semantics else 0,
    ...
    def forward(self, graph_data, ws, shape, noise_mode='random'):
        ...
        for i in range(len(self.num_faces) - 1):
            ...
            x, face_colors = self.blocks[i](sub_neighborhoods, is_pad, pads, pool_maps, x, face_colors,
                                            split_ws[i + 1], shape[len(shape) - 1 - i],
                                            graph_data['semantic_maps'][len(graph_data['semantic_maps']) - 1 - i], noise_mode)
...
class SynthesisBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, w_dim, current_geo_channels, next_geo_channels, num_face, color_channels, semantic_channels, last_block):
        super().__init__()
        ...
        if self.semantic_channels > 0:
            self.conv_pre = SPADEResnetBlock(fin=in_channels - current_geo_channels, fout=in_channels - current_geo_channels,
                                             semantic_nc=self.semantic_channels)
    ...
    def forward(self, face_neighborhood, face_is_pad, pad_size, pool_map, x, img, ws, shape, semantics, noise_mode):
        w_iter = iter(ws.unbind(dim=1))
        if self.semantic_channels > 0:
            x = self.conv_pre(x, face_neighborhood[0], face_is_pad[0], pad_size[0], semantics)
...
class SPADE(torch.nn.Module):
    ...
    def __init__(self, norm_nc, label_nc):
        ...
        self.param_free_norm = BatchNorm(norm_nc, affine=False)

        nhidden = 128

        self.mlp_shared = FaceConvDemodulated(label_nc, nhidden)
        self.mlp_gamma = FaceConvDemodulated(nhidden, norm_nc)
        self.mlp_beta = FaceConvDemodulated(nhidden, norm_nc)

    def forward(self, x, face_neighborhood, face_is_pad, pad_size, segmap):

        normalized = self.param_free_norm(x)

        actv = torch.nn.functional.relu(self.mlp_shared(segmap, face_neighborhood, face_is_pad, pad_size))
        gamma = self.mlp_gamma(actv, face_neighborhood, face_is_pad, pad_size)
        beta = self.mlp_beta(actv, face_neighborhood, face_is_pad, pad_size)
...
class SPADEResnetBlock(torch.nn.Module):
    ...
    def __init__(self, fin, fout, semantic_nc):
        ...
        # create conv layers
        self.conv_0 = FaceConvDemodulated(fin, fmiddle)
        self.conv_1 = FaceConvDemodulated(fmiddle, fout)

        if self.learned_shortcut:
            self.conv_s = torch.nn.Linear(fin, fout, bias=False)
    ...
    def forward(self, x, face_neighborhood, face_is_pad, pad_size, seg):
        seg = torch.nn.functional.one_hot(seg, num_classes=self.semantic_nc).float()
        x_s = self.shortcut(x, face_neighborhood, face_is_pad, pad_size, seg)
        dx = self.conv_0(self.actvn(self.norm_0(x, face_neighborhood, face_is_pad, pad_size, seg)), face_neighborhood, face_is_pad, pad_size)
        dx = self.conv_1(self.actvn(self.norm_1(dx, face_neighborhood, face_is_pad, pad_size, seg)), face_neighborhood, face_is_pad, pad_size)
        out = x_s + dx

        return out

    def shortcut(self, x, face_neighborhood, face_is_pad, pad_size, seg):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, face_neighborhood, face_is_pad, pad_size, seg))
        else:
            x_s = x
        return x_s
    ...
    @staticmethod
    def actvn(x):
        return torch.nn.functional.leaky_relu(x, 2e-1) * activation_funcs['lrelu']['def_gain']
