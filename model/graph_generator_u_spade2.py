...
class SynthesisNetwork(torch.nn.Module):
    ...
    def __init__(self, w_dim, num_faces, color_channels, semantic_channels, channel_base=16384, channel_max=512):
        ...
        for blk_idx, cdict_key in enumerate(self.block_pow_2[1:]):
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, geo_channels=geo_channels, sem_channels=semantic_channels,
    ...
    def forward(self, graph_data, ws, shape, noise_mode='random'):
        ...
        for i in range(len(self.num_faces) - 1):
            ...
            semantics = torch.nn.functional.one_hot(graph_data['semantic_maps'][len(graph_data['semantic_maps']) - 2 - i].long(), num_classes=self.semantic_channels).float()
            x, face_colors = self.blocks[i](sub_neighborhoods, is_pad, pads, pool_maps, x, face_colors, split_ws[i + 1],
                                            shape[len(shape) - 1 - i], semantics, noise_mode)
...
class SynthesisBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, w_dim, geo_channels, sem_channels, num_face, color_channels, last_block):
        super().__init__()
        ...
        self.spade0 = SPADE_IN(out_channels, sem_channels)
        self.conv1 = SynthesisLayer(out_channels, out_channels - geo_channels, w_dim=w_dim, num_face=num_face)
        self.spade1 = SPADE_IN(out_channels - geo_channels, sem_channels)
        self.torgb = ToRGBLayer(out_channels - geo_channels, color_channels, w_dim=w_dim, num_face=num_face)
...
class SemanticsDemodulated(torch.nn.Module):
...
class SPADE_IN(torch.nn.Module):
    ...
    def __init__(self, norm_nc, label_nc):
        super().__init__()
        self.semantic_features = torch.nn.Linear(label_nc, norm_nc)
        self.modulated_conv = SemanticsDemodulated(norm_nc, norm_nc)
