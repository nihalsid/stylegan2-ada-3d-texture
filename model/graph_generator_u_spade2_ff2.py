...
class Generator(torch.nn.Module):
    ...
    def __init__(self, z_dim, w_dim, w_num_layers, num_faces, color_channels, ff2_channels, c_dim=0, channel_base=16384, channel_max=512):
        super().__init__()
        self.z_dim = z_dim
        self.w_dim = w_dim
        self.c_dim = c_dim
        ...
        self.synthesis = SynthesisNetwork(w_dim=w_dim, num_faces=num_faces, color_channels=color_channels, ff2_channels=ff2_channels, channel_base=channel_base, channel_max=channel_max)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def __init__(self, w_dim, num_faces, color_channels, ff2_channels, channel_base=16384, channel_max=512):
        super().__init__()
        self.w_dim = w_dim
        self.num_faces = num_faces
        self.color_channels = color_channels
        ...
        use_ff2_conditioning = {4: False, 8: False, 16: False, 32: True, 64: True, 128: True}
        self.blocks = torch.nn.ModuleList()
        block_level = len(self.block_pow_2) - 1
        self.num_ws = 2 * len(self.block_pow_2)
        self.ff2_channels = ff2_channels
        ...
        for blk_idx, cdict_key in enumerate(self.block_pow_2[1:]):
            ...
            ff2_conditioning = use_ff2_conditioning[cdict_key]
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, geo_channels=geo_channels, ff2_channels=ff2_channels if ff2_conditioning else 0,
    ...
    def forward(self, graph_data, ws, shape, noise_mode='random'):
        ...
        for i in range(len(self.num_faces) - 1):
            ...
            x, face_colors = self.blocks[i](sub_neighborhoods, is_pad, pads, pool_maps, x, face_colors, split_ws[i + 1],
                                            shape[len(shape) - 1 - i], graph_data['ff2_maps'][len(graph_data['ff2_maps']) - 2 - i], noise_mode)
...
class SynthesisBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, w_dim, geo_channels, ff2_channels, num_face, color_channels, last_block):
        super().__init__()
        ...
        self.torgb = ToRGBLayer(out_channels - geo_channels, color_channels, w_dim=w_dim, num_face=num_face)
        if self.use_ff2:
            self.spade0 = SPADE_IN(out_channels, ff2_channels)
            self.spade1 = SPADE_IN(out_channels - geo_channels, ff2_channels)
        self.last_block = last_block

    def forward(self, face_neighborhood, face_is_pad, pad_size, pool_map, x, img, ws, shape, ff2, noise_mode):
        w_iter = iter(ws.unbind(dim=1))
...
class FF2Demodulated(torch.nn.Module):
    ...
    def forward(self, x, s):
        ...
        w2 = w * w
        s2 = s * s
        denominator = torch.nn.functional.linear(s2, w2).mean(0)
        numerator = torch.nn.functional.linear(x * s, w)
        out = numerator / denominator[None, :]
        return torch.clamp(out + self.bias[None, :], -256, 256)
...
class SPADE_IN(torch.nn.Module):
    ...
    def __init__(self, norm_nc, label_nc):
        super().__init__()
        self.ff2_features = torch.nn.Linear(label_nc, norm_nc)
        self.modulated_conv = FF2Demodulated(norm_nc, norm_nc)

    def forward(self, x, segmap):
        ff2_features = self.ff2_features(segmap)
        out = self.modulated_conv(x, ff2_features)
        return out
