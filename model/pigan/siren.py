...
from model import normalize_2nd_moment
...
class CustomMappingNetwork(nn.Module):
    ...
    def __init__(self, z_dim, shape_dim, map_hidden_dim, map_output_dim):
        super().__init__()
        self.shape_net = nn.Sequential(nn.Linear(shape_dim, z_dim), nn.LeakyReLU(0.2, inplace=True))
        self.network = nn.Sequential(nn.Linear(z_dim * 2, map_hidden_dim),
...
class TALLSIREN(nn.Module):
    ...
    def __init__(self, input_dim=2, z_dim=256, hidden_dim=256, shape_dim=256, device=None):
        super().__init__()
        self.device = device
