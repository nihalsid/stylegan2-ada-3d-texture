...
class SmoothUpsampleSparse(torch.nn.Module):

    def __init__(self):
        super().__init__()
        self.interpolator = ME.MinkowskiInterpolation()

    def forward(self, x_sparse, x_field):
        x_field = x_field.float()
        halved_field = torch.cat([x_field[:, 0:1], x_field[:, 1:2] / 2, x_field[:, 2:3] / 2, x_field[:, 3:4] / 2], 1)
        x_interp = self.interpolator(x_sparse, halved_field)
        return x_interp
...
class MinkowskiModulatedConvolution(ME.MinkowskiModuleBase):
    ...
    def __init__(
        ...
        super(MinkowskiModulatedConvolution, self).__init__()
        self.is_transpose = False
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.dimension = dimension
        self.use_mm = False
        self.kernel_generator = ME.KernelGenerator(
        ...
        self.kernel = torch.nn.Parameter(torch.randn(*kernel_shape))
        self.bias = torch.nn.Parameter(torch.zeros(1, out_channels)) if bias else None
        self.convolution_mode = ConvolutionMode.DEFAULT
        self.conv = ME.MinkowskiConvolutionFunction()

    def forward(self, feats, coords, styles, demodulate=True):
        batch_size = styles.shape[0]

        # calculate dcoefs if required
        dcoefs = None
        if demodulate:
            w = self.kernel.unsqueeze(0)  # 1 x K^3 x N_in x N_out
            w = w * styles.reshape(batch_size, 1, -1, 1)  # B x 1 x N_in x 1
            dcoefs = (w.square().sum(dim=[1, 2]) + 1e-8).rsqrt()  # B x N_out
        feats = feats * styles[coords[:, 0].long(), :]
        input = ME.SparseTensor(feats, coords.int())
        if self.use_mm:
            ...
            out_coordinate_map_key = ME._get_coordinate_map_key(
                input, None, self.kernel_generator.expand_coordinates
            )
        ...
        if demodulate:
            outfeat = outfeat * dcoefs[coords[:, 0].long(), :]
