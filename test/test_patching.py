...
def test_patches(config):
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        ...
        patches = StyleGAN2Trainer.extract_patches_from_tensor(batch['real_hres'][first_views], batch['mask_hres'][first_views, 0, :, :], config.num_patch_per_view * config.views_per_sample, config.patch_size)
        ...
        rend_patches = StyleGAN2Trainer.extract_patches_from_tensor(rendered_color_gt[:, :3, :, :], 1 - rendered_color_gt[:, 3, :, :], config.num_patch_per_view, config.patch_size)
        ...
        save_image(torch.cat([patches, rend_patches], dim=0), f"runs/patches/{batch_idx:04d}_patch.png", nrow=config.batch_size, value_range=(-1, 1), normalize=True)
