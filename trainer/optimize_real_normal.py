...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.train_set = FaceGraphMeshDataset(config, single_mode=True)
        self.val_set = FaceGraphMeshDataset(config, config.num_eval_images, single_mode=True)
    ...
    def export_grid(self, prefix, output_dir_vis, output_dir_fid):
        ...
        grid_loader = iter(GraphDataLoader(self.val_set, batch_size=self.config.batch_size))
