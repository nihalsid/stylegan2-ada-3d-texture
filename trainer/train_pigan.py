...
from model.pigan import fid_evaluation
...
from model.pigan.discriminator import ProgressiveEncoderDiscriminator
...
def train(rank, opt):
    ...
    if opt.load_dir != '':
        ...
        generator = TALLSIREN(3, z_dim=metadata['latent_dim'], hidden_dim=512, shape_dim=256).to(device)
        discriminator = ProgressiveEncoderDiscriminator().to(device)
        ...
        for i, batch in enumerate(dataloader):
            ...
            if rank == 0:
                ...
                if discriminator.step % opt.sample_interval == 0:
                    ...
                    save_image(gen_imgs[:25], os.path.join(opt.output_dir, f"{discriminator.step}_fixed.png"), nrow=4, normalize=True)
                    ...
                    with torch.no_grad():
                        ...
                        copied_metadata['img_size'] = 128
                        gen_imgs = render(generator(eval_faces, fixed_z.to(device), eval_shape), eval_sample, copied_metadata['img_size'])
                    save_image(gen_imgs[:25], os.path.join(opt.output_dir, f"{discriminator.step}_fixed_ema.png"), nrow=4, normalize=True)
                ...
                with torch.no_grad():
                    ...
                    while img_counter < fid_num_imgs:
                        ...
                        for img in generated_imgs:
                            save_image(img, os.path.join(generated_dir, f'{img_counter:0>5}.jpg'), normalize=True)
                            img_counter += 1
                            pbar.update(1)
                pbar.close()
