...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.S = GraphEncoder(self.train_set.num_feats_1, layer_dims=layer_dims)
        self.R = None
    ...
    def configure_optimizers(self):
        g_opt = torch.optim.Adam([
            ...
            {'params': list(self.S.parameters()), 'lr': self.config.lr_e, 'eps': 1e-8, 'weight_decay': 1e-4}
        ])
    ...
    def forward(self, batch, limit_batch_size=False):
        ...
        fake = self.G.synthesis(batch['graph_data'], w, batch['shape'], batch['semantics'])
        return fake, w
    ...
    def g_regularizer(self, batch):
        ...
        for idx in range(len(batch['shape'])):
            ...
            batch['semantics'][idx] = batch['semantics'][idx].detach()
        g_opt.zero_grad(set_to_none=True)
        fake, w = self.forward(batch)
    ...
    def validation_epoch_end(self, _val_step_outputs):
        ...
        with Timer("export_samples"):
            ...
            for iter_idx, batch in enumerate(self.val_dataloader()):
                ...
                fake_render = self.render(self.G(batch['graph_data'], latents[iter_idx % len(latents)].to(self.device), shape, semantics, noise_mode='const'), batch, use_bg_color=False).cpu()
    ...
    def set_shape_codes(self, batch):
        code_0 = self.E(batch['x_0'], batch['graph_data'])
        code_1 = self.S(batch['x_1'], batch['graph_data'])
        batch['shape'] = code_0
        batch['semantics'] = code_1
    ...
    def export_grid(self, prefix, output_dir_vis, output_dir_fid):
        ...
        for iter_idx, z in enumerate(self.grid_z.split(self.config.batch_size)):
            ...
            fake = self.render(self.G(eval_batch['graph_data'], z, eval_batch['shape'], eval_batch['semantics'], noise_mode='const'), eval_batch, use_bg_color=False).cpu()
    ...
    def export_mesh(self, outdir):
        ...
        for iter_idx, z in enumerate(self.grid_z.split(self.config.batch_size)):
            if iter_idx < self.config.num_vis_meshes // self.config.batch_size:
                ...
                eval_batch = to_device(next(grid_loader), self.device)
                self.set_shape_codes(eval_batch)
                generated_colors = torch.clamp(self.G(eval_batch['graph_data'], z, eval_batch['shape'], eval_batch['semantics'], noise_mode='const'), -1, 1)
