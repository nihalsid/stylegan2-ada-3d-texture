...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.E = TwinGraphEncoder(self.train_set.num_feats, 1, layer_dims=(64, 128, 128, 256, 256, 256, 384, 384), conv_layer=TextureConv)
    ...
    def g_step(self, batch, resolution, alpha):
        g_opt = self.optimizers()[0]
        g_opt.zero_grad(set_to_none=True)
        fake, w = self.forward(batch)
        p_fake = self.D(self.augment_pipe(self.render(fake, batch, resolution=resolution)), alpha)
    ...
    def g_regularizer(self, batch, resolution):
        g_opt = self.optimizers()[0]
        for idx in range(len(batch['shape'])):
            batch['shape'][idx] = batch['shape'][idx].detach()
        g_opt.zero_grad(set_to_none=True)
        fake, w = self.forward(batch)
        plp = self.path_length_penalty(self.render(fake, batch, resolution=resolution), w)
    ...
    def d_step(self, batch, resolution, alpha):
        d_opt = self.optimizers()[1]
        d_opt.zero_grad(set_to_none=True)

        fake, _ = self.forward(batch)
        p_fake = self.D(self.augment_pipe(self.render(fake.detach(), batch, resolution=resolution)), alpha)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print(self.global_step, resolution, alpha)
        set_real_sample_resolution(batch, resolution)

        # optimize generator
        self.g_step(batch, resolution, alpha)

        if self.config.lazy_path_penalty_interval != -1 and self.global_step > self.config.lazy_path_penalty_after and (self.global_step + 1) % self.config.lazy_path_penalty_interval == 0:
            self.g_regularizer(batch, resolution)
        ...
        self.d_step(batch, resolution, alpha)
    ...
    def update_state(self):
        if not self.state.item() == (len(self.config.progressive_switch) - 1):
            if self.global_step > self.config.progressive_switch[self.state.item()]:
                self.state += 1
