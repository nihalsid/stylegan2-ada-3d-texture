...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.D = DiscriminatorProgressive(config.image_size, 3, w_num_layers=config.num_mapping_layers, mbstd_on=config.mbstd_on, channel_base=config.d_channel_base)
        ...
        self.ema = None
        state_dict = torch.load(config.prog_resume, map_location=torch.device("cpu"))["state_dict"]
        self.G.load_state_dict(get_parameters_from_state_dict(state_dict, "G"))
        self.E.load_state_dict(get_parameters_from_state_dict(state_dict, "E"))
        # load checkpoint D
        self.D.fromRGB['128'].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.0"))
        self.D.module_list[1].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.1"))
        self.D.module_list[2].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.2"))
        self.D.module_list[3].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.3"))
        self.D.module_list[4].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.4"))
        self.D.module_list[5].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.5"))
        self.D.module_list[6].load_state_dict(get_parameters_from_state_dict(state_dict, "D.module_list.6"))
    ...
    def g_step(self, batch, alpha):
        g_opt = self.optimizers()[0]
        g_opt.zero_grad(set_to_none=True)
        fake, w = self.forward(batch)
        p_fake = self.D(self.augment_pipe(self.render(fake, batch)), alpha)
    ...
    def d_step(self, batch, alpha):
        d_opt = self.optimizers()[1]
        d_opt.zero_grad(set_to_none=True)

        fake, _ = self.forward(batch)
        p_fake = self.D(self.augment_pipe(self.render(fake.detach(), batch)), alpha)
        fake_loss = torch.nn.functional.softplus(p_fake).mean()
        self.manual_backward(fake_loss)

        p_real = self.D(self.augment_pipe(self.train_set.get_color_bg_real(batch)), alpha)
    ...
    def d_regularizer(self, batch, alpha):
        d_opt = self.optimizers()[1]
        d_opt.zero_grad(set_to_none=True)
        image = self.train_set.get_color_bg_real(batch)
        image.requires_grad_()
        p_real = self.D(self.augment_pipe(image, True), alpha)
    ...
    def on_train_start(self):
        if self.ema is None:
            self.ema = torch.load(self.config.prog_resume_ema, map_location=self.device)
        if self.R is None:
    ...
    def get_alpha(self):
        alpha = max(0, min(1, self.global_step / self.config.alpha_switch[0]))
