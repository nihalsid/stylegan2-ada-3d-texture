...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.G = Generator(config.latent_dim, config.latent_dim, config.num_mapping_layers, config.num_faces, 3, c_dim=256, channel_base=config.g_channel_base, channel_max=config.g_channel_max)
    ...
    def get_mapped_latent(self, z, c, style_mixing_prob):
        if torch.rand(()).item() < style_mixing_prob:
            ...
            w1 = self.G.mapping(z[0], c=c)[:, :cross_over_point, :]
            w2 = self.G.mapping(z[1], c=c, skip_w_avg_update=True)[:, cross_over_point:, :]
