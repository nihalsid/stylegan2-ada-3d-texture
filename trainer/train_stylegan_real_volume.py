...
from dataset.mesh_real_volume import FaceGraphMeshDataset
...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def g_step(self, batch):
        ...
        p_fake = self.D(self.augment_pipe(self.render(fake, batch)), self.to_D_condition(batch['condition']).detach())
    ...
    def d_step(self, batch):
        ...
        p_fake = self.D(self.augment_pipe(self.render(fake.detach(), batch)), self.to_D_condition(batch['condition']).detach())
        fake_loss = torch.nn.functional.softplus(p_fake).mean()
        self.manual_backward(fake_loss)

        p_real = self.D(self.augment_pipe(batch['real']), self.to_D_condition(batch['condition']).detach())
    ...
    def d_regularizer(self, batch):
        ...
        p_real = self.D(self.augment_pipe(image, True), self.to_D_condition(batch['condition']).detach())
    ...
    def set_condition_code(self, batch):
        code = self.E(batch['df'])
        b, c, d, h, w = code.shape
        code = code.reshape(b, c * d * h * w)
    ...
    def export_grid(self, prefix, output_dir_vis, output_dir_fid):
        ...
        for iter_idx, z in enumerate(self.grid_z.split(self.config.batch_size)):
            ...
            self.set_condition_code(eval_batch)
